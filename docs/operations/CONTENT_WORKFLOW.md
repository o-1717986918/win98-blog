...
pnpm content:new column stable-kebab-slug "栏目标题"
```
...
| `draft` | `true` 时仅开发预览可见 |
| `tags` | 主题标签；自动生成标签目录 |
| `columns` | 栏目 ID 数组；至少保留一个真实栏目引用 |
| `cover` | `{ src, alt }`；相对路径由 Astro 图片管线处理 |
| `chrome` | `full / minimal / none`，只表示装配的外壳资源 |
| `theme` | `graphite / paper / night / indigo` |
| `featured` | 精选标记，供策展排序扩展使用 |
| `comments` | `inherit / enabled / disabled`；外部 provider 未配置时仍不加载 |
| `noindex` | 输出搜索引擎 robots 指令 |
| `license` | 文章页尾许可说明 |

栏目使用相同的 `chrome/theme/cover/draft`，另有 `nav`、`navLabel`、`order` 与 `showPosts`。栏目不是文章层级，文章可以同时属于多个栏目。
...
- 发布前依次执行 `pnpm content:audit` 与 `pnpm verify`。

## 4. 审查清单

1. 标题和摘要能独立表达文章价值，没有占位文案；
2. 内部链接存在，栏目引用有效，图片都有具体替代文本；
3. 标题层级连续，表格和代码块在窄屏可滚动；
4. `none` 页面只显式导入自己需要的资源；
