...
- Pagefind 自定义 API 支持用自定义的轻量界面呈现即时结果；搜索最终收纳为顶栏中的单一输入框，由点击、`/` 或 `Ctrl/⌘ K` 聚焦，结果作为顶栏下方的非模态暂态层出现。独立 `/search/` 只保留为兼容说明页。
...
- 任意 full 页面都可通过顶栏单一输入框或键盘搜索；支持即时结果、方向键浏览与本地 Pagefind 索引；
